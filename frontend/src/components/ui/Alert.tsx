...
export default function Alert({ variant = 'info', title, children, className, onClose }: AlertProps) {
  ...
  return (
    <div className={cn('rounded-xl border p-4', classes.container, className)}>
      <div className="flex">
        ...
        <div className="ml-3 flex-1">
          ...
          <div className={cn('text-sm', title && 'mt-1', classes.content)}>
            {children}
          </div>
        </div>
        {onClose && (
          <button
            onClick={onClose}
            className={cn('ml-3 flex-shrink-0 p-1 rounded-lg hover:bg-black/5 transition-colors', classes.icon)}
          >
